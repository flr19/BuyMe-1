...
try{
	...
	if (name != null && name.length() > 0) {
	    rs = st.executeQuery("SELECT * FROM account WHERE username LIKE'%"+name+"%' AND username IN (SELECT username FROM customerrep)");
	} else {
...
%>
	<tr>
		...
		<td>
			<form method="POST">
      			<input type="submit" value="Delete" onclick="if(confirm('Are you sure? This action cannot be undone.')){form.action='deleteCustomerRep.jsp'}">
